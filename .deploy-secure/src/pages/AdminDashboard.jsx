...
import {
  ...
  getEnquiryById,
  getFleet,
  updateAdminFeedback as updateAdminFeedbackRecord,
  updateEnquiry,
  updateEnquiryNotes,
  updateEnquiryQuote,
  updateEnquiryStatus,
...
export default function AdminDashboard() {
  ...
  const syncOperationalData = async ({ showLoader = false } = {}) => {
    ...
    try {
      const [nextEnquiries, nextFeedbackEntries, nextFleet, nextDrivers] = await Promise.all([
        getEnquiries(),
        getAdminFeedback(),
        getFleet(),
        getDrivers(),
  ...
  const filteredFeedback = useMemo(() => {
    const normalizedSearch = feedbackFilters.search.trim().toLowerCase();

    return feedbackEntries.filter((feedback) => {
      const matchesStatus = feedbackFilters.status === 'all' || feedback.status === feedbackFilters.status;
      const matchesFeatured = feedbackFilters.featured === 'all'
        || (feedbackFilters.featured === 'featured' && feedback.featured)
        || (feedbackFilters.featured === 'standard' && !feedback.featured);
      const matchesSearch = !normalizedSearch || [
        feedback.reference_id,
        feedback.customer_name,
        feedback.display_name,
        feedback.city,
        feedback.display_city,
        feedback.service_used,
        feedback.display_service_used,
      ].some((value) => String(value || '').toLowerCase().includes(normalizedSearch));

      return matchesStatus && matchesFeatured && matchesSearch;
    });
  }, [feedbackEntries, feedbackFilters]);
  ...
  const normalizedShowHeaderSearch = !['dashboard', 'feedback'].includes(activeTab);
  ...
  const handleSaveFeedback = async (feedbackId, payload) => {
    ...
    try {
      const updated = await updateAdminFeedbackRecord(feedbackId, payload);
      setFeedbackEntries((current) => current.map((item) => (
        String(item.id) === String(updated.id) ? updated : item
      )));
      return updated;
    } catch (error) {
      const message = error.message || 'Unable to update feedback. Please try again.';
      setDashboardError(message);
      throw new Error(message);
    }
  };
  ...
  return (
    <>
      ...
      <DesktopAdminDashboard
        ...
        adminProfile={adminProfile}
        handleLogout={handleLogout}
        showHeaderSearch={normalizedShowHeaderSearch}
